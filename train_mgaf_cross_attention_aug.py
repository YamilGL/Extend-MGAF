# 7_train_mgaf_cv.py (CON ATENCIÓN CRUZADA)
import torch
...
MGAF_SAVE_PATH = 'models/mgaf_model_cv_final_cross_attention.pth' # Nombre de archivo actualizado
...
# --- Dataset Personalizado (Lógica sin cambios) ---
...
# --- CAMBIO 1: IMPLEMENTACIÓN DEL BLOQUE DE ATENCIÓN CRUZADA ---
# Se reemplaza el GAF_Block original por este nuevo módulo.
class CrossAttentionGAF(nn.Module):
    """
    Implementa un bloque de Atención Cruzada (Cross-Attention) para fusionar
    dos mapas de características (profundidad e inercial).
    La atención para cada modalidad se genera a partir de la otra modalidad.
    """
    def __init__(self, channels):
        super(CrossAttentionGAF, self).__init__()
        # Capa que genera el mapa de atención para el flujo de PROFUNDIDAD,
        # usando las características INERCIALES como contexto.
        self.attn_gen_for_depth = nn.Conv2d(channels, channels, kernel_size=1)
        
        # Capa que genera el mapa de atención para el flujo INERCIAL,
        # usando las características de PROFUNDIDAD como contexto.
        self.attn_gen_for_inertial = nn.Conv2d(channels, channels, kernel_size=1)
        
        self.sigmoid = nn.Sigmoid()

    def forward(self, f_depth, f_inertial):
        # 1. Generar el mapa de atención para el flujo de profundidad.
        #    Se usa f_inertial para decidir a qué prestar atención en f_depth.
        attn_map_for_depth = self.sigmoid(self.attn_gen_for_depth(f_inertial))
        
        # 2. Generar el mapa de atención para el flujo inercial.
        #    Se usa f_depth para decidir a qué prestar atención en f_inertial.
        attn_map_for_inertial = self.sigmoid(self.attn_gen_for_inertial(f_depth))
        
        # 3. Aplicar los mapas de atención (atención cruzada).
        attended_depth = f_depth * attn_map_for_depth
        attended_inertial = f_inertial * attn_map_for_inertial
        
        # 4. Fusionar los flujos atendidos (la suma es una forma común de fusión).
        fused_features = attended_depth + attended_inertial
...
class MGAFModel(nn.Module):
    def __init__(self, num_classes, dropout_rate=0.3):
        ...
        # Flujo de Profundidad (sin cambios)
        ...
        # --- CAMBIO 2: INSTANCIAR EL NUEVO BLOQUE DE ATENCIÓN CRUZADA ---
        self.gaf_fusion = CrossAttentionGAF(channels=32)
        # --- FIN DEL CAMBIO 2 ---

        # Clasificador (sin cambios)
    ...
    def forward(self, x_depth, x_inertial):
        ...
        # La llamada al bloque de fusión no cambia, es polimórfica
        fused = self.gaf_fusion(f_depth, f_inertial)
        output = self.classifier(fused)
        return output

# --- El resto del código (funciones de carga, entrenamiento, validación y main) permanece EXACTAMENTE IGUAL ---
...
def main():
    ...
    criterion = nn.CrossEntropyLoss()
    for epoch in tqdm(range(EPOCHS), desc="Entrenamiento final del modelo MGAF con Cross-Attention"):
    ...
    print(f"💾 Modelo final MGAF con Cross-Attention guardado en: {MGAF_SAVE_PATH}")
