...
class FeatureReducer(nn.Module):
    def __init__(self, input_dim, reduced_dim):
        ...
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(256, reduced_dim)
...
reducer = FeatureReducer(X_train.shape[1], 64)
reducer.eval()  # no entrenamiento porque usas como extractor

with torch.no_grad():
    X_train_reduced = reducer(torch.tensor(X_train, dtype=torch.float32)).numpy()
    X_val_reduced = reducer(torch.tensor(X_val, dtype=torch.float32)).numpy()

# Sustituye X_train y X_val por los reducidos
